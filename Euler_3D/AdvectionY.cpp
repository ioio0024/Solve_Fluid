...
void Fluid3D::AdvectionY(Int32 nz, Int32 nx,
                         const Vector3DR &d_p,
                         const Vector3DR &u_p,
                         const Vector3DR &v_p,
                         const Vector3DR &w_p,
                         ...
                         Vector3DR *p_vn) noexcept
{
  ...
  for(auto k = GHOST; k < nz - GHOST; ++k){
    for(auto j = GHOST; j < ny_ + 1 - GHOST; ++j){
      for(auto i = GHOST; i < nx - GHOST; ++i){
        ...
        AdvCipRK3Y(0, i, j, k, dens, velx, vely, velz, pres,
                   d_p, u_p, v_p, w_p, p_p,
                   d_v, u_v, v_v, w_v, p_v);
        AdvCipRK3Y(1, i, j, k, dens, velx, vely, velz, pres,
                   d_p, u_p, v_p, w_p, p_p,
                   d_v, u_v, v_v, w_v, p_v);
        AdvCipRK3Y(2, i, j, k, dens, velx, vely, velz, pres,
                   d_p, u_p, v_p, w_p, p_p,
                   d_v, u_v, v_v, w_v, p_v);
  ...
  for(auto k = GHOST; k < nz - GHOST; ++k){
    for(auto j = GHOST; j < ny_ - GHOST; ++j){
      for(auto i = GHOST; i < nx - GHOST; ++i){
        auto dy = y_[j + 1] - y_[j];
        (*d_vn)(i, j, k) = d_v(i, j, k) - dt_ / dy
            * (dflux_(i, j + 1, k) - dflux_(i, j, k));
        (*u_vn)(i, j, k) = (d_v(i, j, k) * u_v(i, j, k) - dt_ / dy
                         * (lflux_(i, j + 1, k) - lflux_(i, j, k)))
                         / (*d_vn)(i, j, k);
        (*v_vn)(i, j, k) = (d_v(i, j, k) * v_v(i, j, k) - dt_ / dy
                         * (mflux_(i, j + 1, k) - mflux_(i, j, k)))
                         / (*d_vn)(i, j, k);
        (*w_vn)(i, j, k) = (d_v(i, j, k) * w_v(i, j, k) - dt_ / dy
                         * (nflux_(i, j + 1, k) - nflux_(i, j, k)))
                         / (*d_vn)(i, j, k);
        (*p_vn)(i, j, k) = p_v(i, j, k) + (gamma_ - 1.0) * (0.5 * (
                    ...
                    - (*d_vn)(i, j, k) * ((*u_vn)(i, j, k) * (*u_vn)(i, j, k)
                      ...
                      - dt_ / dy * (eflux_(i, j + 1, k) - eflux_(i, j, k));
      }
    }
  }
...
void Fluid3D::AdvCipRK3Y(Int32 l, Int32 i, Int32 j, Int32 k,
                         ...
                         const Vector3DR &p_v) noexcept
{
  ...
  //lambda 0
  rk[0] = CipCsl3Y(i, j, k, lambda[0], d_p, d_v);
  uk[0] = CipCsl3Y(i, j, k, lambda[0], u_p, u_v);
  wk[0] = CipCsl3Y(i, j, k, lambda[0], w_p, w_v);
  pk[0] = CipCsl3Y(i, j, k, lambda[0], p_p, p_v);

  //lambda 1
  vk[1] = CipCsl3Y(i, j, k, lambda[1], v_p, v_v);
  pk[1] = CipCsl3Y(i, j, k, lambda[1], p_p, p_v);

  //lambda 2
  vk[2] = CipCsl3Y(i, j, k, lambda[2], v_p, v_v);
  pk[2] = CipCsl3Y(i, j, k, lambda[2], p_p, p_v);
